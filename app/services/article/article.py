...
def get_all_articles_except_user_articles(db: Session, user_id: UUID, skip: int = 0, limit: int = 10):
...
def update_article(db: Session, article_id: UUID, article: ArticleUpdate, current_user: User):
...
def delete_article(db: Session, article_id: UUID, current_user: User):
    db_article = db.query(Article).filter(Article.id == article_id, Article.author_id == current_user.id).first()
    if not db_article:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Article not found")
